...
class NatcomIso8583(ISO8583):
  ...
  def __init__(self):
     ...
     ISO8583._BITS_VALUE_TYPE[64] = ['64', 'Signature', 'LLL', 999, 'anb']
...
def verifySignature(textstr, sig):
    ...
    #if verifier.verify(h, sgNo64):
    #   slog( "The signature is authentic.")
    #   return True
    #else:
    #   slog( "The signature is not authentic - hash key size<%d> " % len(sgNo64))
    #   return False
