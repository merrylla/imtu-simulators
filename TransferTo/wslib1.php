...
//errlog("DEBUG: processRequest for $key and", $value);
       if (!isset($value)) continue;

       if (preg_match('/^x_/', $key)) {
              $k=preg_replace('/^x_(.*$)/', '${1}', $key);
//errlog("DEBUG: processRequest returning $k => ", $value);
...
//errlog("DEBUG: processRequest returning ", $ret);
