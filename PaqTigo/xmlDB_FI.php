...
function insertRecord($db, $key, $amt, $msisdn, $pin, $refID, $srcAmt, $tgtAmt, 
                       $retCode, $retMess, $commCredSrc, $commDebSrc, 
                       $commCredTgt, $commDebTgt, $delay)
  { 
    deleteRecord($db, $key);

    $stm1 = sprintf("INSERT INTO IMTU_Data 
      (x_account_number, x_Amount, x_sourceMSISDN, x_PIN, r_ReferenceID,
       r_amountSource, r_amountTarget, r_ReturnCode, r_ReturnMessage,
       r_sourceCommissionCredit, r_sourceCommissionDebit, 
	r_targetCommissionCredit, r_targetCommissionDebit, delay)
 VALUES('%s','%s', '%s', '%s', '%s','%s', '%s', '%s', '%s','%s','%s','%s','%s','%s' 
         )", $key, $amt, $msisdn, $pin, $refID, $srcAmt, $tgtAmt, $retCode, 
             $retMess, $$commCredSrc, $commDebSrc, $commCredTgt, $commDebTgt,
             $delay);
...
function insertBalRec($db, $key, $currType, $localCurr, $sym, $date, 
                       $retCode, $retMess, $bal, $delay){ 
    deleteRecord($db, $key);

    $stm1 = sprintf("INSERT INTO IMTU_Data 
      (x_account_number, r_CurrencyType, r_LocalCurrency, r_symbol, x_DateTo,
       r_ReturnCode, r_ReturnMessage, balance, delay)
 VALUES('%s', '%s','%s', '%s', '%s', '%s','%s','%s','%s'
         )", $key, $currType, $localCurr, $sym, $date, $retCode, $retMess, 
             $bal, $delay);
...
FILOG("xmlDB_FI.php::nodeName: " . $node->nodeName . " nodeValue: " . $node->nodeValue . "\n");
    switch ($node->nodeName) {
       ...
       case 'retMess3':
        ...
        break;
       case 'commCredSrc':
        $commCredSrc = $node->nodeValue;
        break;
       case 'commDebSrc':
        $commDebSrc = $node->nodeValue;
        break;
       case 'commCredTgt':
        $commCredTgt = $node->nodeValue;
        break;
       case 'commDebTgt':
        $commDebTgt = $node->nodeValue;
        break;
       case 'date':
        $date = $node->nodeValue;
        break;
...
FILOG("xmlDB_FI.php::handler: cmd is: " . $cmd . " key is: " . $key . " data is: " . $data . "\n");
    ...
    switch ($cmd) {
       ...
       case 'INSERT':
           ...
           if (isset($bal)) {
              insertBalRec($db, $key, $currType, $localCurr, $sym, $date, 
                       $retCode, $retMess, $bal, $delay); 
		
           } else insertRecord($db, $key, $amt, $msisdn, $pin, $refID, $srcAmt,  
                       $tgtAmt, $retCode, $retMess, $commCredSrc, $commDebSrc, 
                       $commCredTgt, $commDebTgt, $delay);
