...
$eA=array();
$dom = new DOMDocument();
   //FILOG("retrievXmlParm2A: Top XML". $xmlinput."\n");
   
  //FILOG("retrievXmlParm2A: Top params" . $paramList . "\n");
if (!$dom->loadXML($xmlinput) ) {
   //FILOG("retrievXmlParm2A: Bad XML". $xmlinput."\n");
    return $rA;
    }
$xmlparams = $dom->getElementsByTagName('*');  //find all nodes

   //FILOG("retrievXmlParm2A: Top"."\n");

foreach ($xmlparams as $node) {
   //FILOG("node: ".$node->getNodePath()."\n");
   foreach($paramList as $param) {

       $nodeName=$node->tagName;
    //    FILOG($nodeName." ".$param."\n");
        $nodeName=preg_replace("/.*:(.*)/", "$1", $nodeName);
      if ($nodeName == $param )
...
FILOG("wslib1.php::processRequestX: member is: " . $member . " data is: " . $data . gettype($reqX) . "\n");
		$req[$member]=$data;
		} // end foreach
    }
    else
