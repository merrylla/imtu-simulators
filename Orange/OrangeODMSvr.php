...
include '../TransferTo/wslib1.php';
include '../QAlib/soaplib.php';
include 'OrangeODM_data.php';

$sessName='IDT-ORANG-SESSION';
$IDTOrangSession=99124;
...
{
    global $IDTOrangeSession;
    session_id($IDTOrangeSession);
...
function popReqToRsp(&$tuData, $p)
{
       ...
       #new tuData updates go here
       if ($p['product'] == '10' ) {
          $tuData->r_pin_based = "yes";
          $tuData->r_pin_code = "123abc";
          $tuData->r_pin_ivr = 7325551212; 
          $tuData->r_pin_serial = "123serial";
       } else if ($p['product'] == '15' ) {
          $tuData->r_pin_based = "no";
          $tuData->r_pin_code = "15prod";
          $tuData->r_pin_ivr = 9085551212; 
          $tuData->r_pin_serial = "1513serial";
       } 
       $tuData->r_pin_validity = "";
       $tuData->r_pin_option_1 = "";
       $tuData->r_pin_option_2 = "";
       $tuData->r_pin_option_3 = "";
...
errlog("DEBUG: Retriving from session - for key: ", $k);
     ...
     if (isset($_SESSION[$k])) { 
        ...
        $xmld=getDB('Orange', $key);
        errlog("DEBUG: Retriving from xmld - for key $key: ", $xmld);
        if (!$xmld) {
           $tid=0;
           $errCode=219;
           $errTxt='Key does not exist';
        } else {
          errlog("INFO get_id_from_key xmld ", $xmld);
          $tid=$key;
          $errCode=0;
          $errTxt='Transaction successful';
        }
        }
   $rspA=array('xml' => array( 'status_code' => 0,
                                            'error_code'  => $errCode,
                                            'error_txt'   => $errTxt,
                                            'id'    => $tid,
                                            'from_key' => $p['from_key'],
...
function balReq($p)
{
   ...
   $errA=array('xml' => array('status_code' => 1,
                                      'error_code'  => $code,
                                      'error_txt'   => $msg   )
              );
...
function topUpReq($p)
{
     ...
     if (!array_key_exists("return_service_fee", $p)) {
        $p["return_service_fee"] = 0;
     }
     ...
     } else  { // save key with destination_msisdn
        $xA=array('a0' => $p['key'],
                  'a1' => $p['destination_msisdn'],
		  'a2' => $p['md5'],
		  'a3' => $p['product'],
		  'a4' => $p['msisdn'],
                  'a5' => $p['operatorid'],
		  'a6' => $p['destination_msisdn'],
                  'a7' => $p['return_service_fee']);
        $keyPut = array('app' => 'Orange', 'key' => $p['key']);
        // Insert into DB so that status can respond correctly when 
        // response takes too long
        insertTransDB($keyPut, $xA);
     }
     //should be same as: AgentId=9999
     // error 104 => msisdn in blacklist
     $prod = $p['product'] ;
     $msisdnLen = strlen ($p['destination_msisdn']);
     errlog("INFO: topUpReq - msisdnLen=", $msisdnLen);
     $prod10 = strcmp($prod, "10");
     $prod15 = strcmp($prod, "15");
     errlog("INFO: topUpReq - prod10 =", $prod10);
     if ((($prod10 == 0) || ($prod15 == 0)) && ($msisdnLen !== 10) ) {
        errlog("INFO: topUpReq - throwing exception for MSISDN length not 10");
        throw new Exception("Incorrect MSISDN length value");
     }
     $retCode='0';
     $retMsg='Tranaction Success';

     $keyA=array('x_account_number' => $p['destination_msisdn']);
// Now get the data inserted into the DB by the CallGen test script 
    $dbObj=getDBdata1('IMTU_DATA', 'OrgTopUpData', $keyA);
    if ($dbObj == FALSE) {
        errlog("INFO: Orange topUpReq - not using test data from DB with key", $keyA);
        $dbObj = new OrgTopUpData;
        $dbObj->remap();
       }
    ...
    errlog("DEBUG: topUpReq - after popReqToRsp p", $p);


    $rsp= processRequest($p, $dbObj);
    errlog("DEBUG: topUpReq - finished processRequest", $rsp);
...
if (getenv('REQUEST_METHOD') == 'POST') {
  $reqRev= file_get_contents("php://input");
      errlog("INFO: input", $reqRev);
  //$xml = simplexml_load_string($client_data);
...
//echo "<br/>From: ", $url, "<br/>";
   ...
   errlog("INFO: throwing new Exception inside catch", $ex->faultcode);
    throw new Exception($ex->faultstring, $ex->faultcode); 
   errlog("INFO: threw new Exception inside catch", $ex->faultcode);
   }
catch (Exception $e) { // catch the Exception that is not a SoapFault
   errlog("INFO: Caught Exception ", $e);
   $msg=$e->getMessage();
