...
function processRequestX($reqX, $ck_params)
{
    ...
    if (gettype($reqX) == 'object' ) {
       ...
       $req = &$reqX;
    errlog("INFO: processRequestX: finished object search", $reqX);

    foreach ($ck_params as $key => $value) {
       errlog("INFO: processRequestX: key : $key value:", $value);

       if (!array_key_exists($key, $req) ) {
          errlog("SOAP FAULT: missing parameter for key :", $key);
          throw new SoapFault("9998", "Error: missing parameter for $key");
          }
       if (($req[$key] != $value) && ($key != "testName")) {
          errlog("SOAP FAULT: parameter for key $key has incorrect value:$req[$key] and not", $value);
